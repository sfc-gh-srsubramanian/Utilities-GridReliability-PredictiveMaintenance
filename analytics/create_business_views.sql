...
reliability_calcs AS (
    SELECT 
        ...
        ((SELECT SUM(a.CUSTOMERS_AFFECTED) 
          FROM ML.MODEL_PREDICTIONS p
          JOIN RAW.ASSET_MASTER a ON p.ASSET_ID = a.ASSET_ID
          WHERE p.RISK_SCORE >= 51) * 4.2 * 60) / 5800000.0 AS POTENTIAL_SAIDI_POINTS,
        ...
        ((SELECT SUM(a.CUSTOMERS_AFFECTED) 
          FROM ML.MODEL_PREDICTIONS p
          JOIN RAW.ASSET_MASTER a ON p.ASSET_ID = a.ASSET_ID
          WHERE p.RISK_SCORE >= 51)) / 5800000.0 AS POTENTIAL_SAIFI_POINTS,
        
        -- Fleet health score (0-100, inverse of average risk)
        100 - (SELECT AVG(RISK_SCORE) FROM ML.MODEL_PREDICTIONS) AS FLEET_HEALTH_SCORE
...
SELECT 
    ...
    CONCAT('$', ROUND(NET_COST_AVOIDANCE/1000000, 2), 'M') AS VALUE
FROM VW_COST_AVOIDANCE_REPORT
UNION ALL
SELECT 
    'ROI',
    CONCAT(ROUND((NET_COST_AVOIDANCE/PREVENTIVE_MAINTENANCE_COST)*100, 0), '%')
FROM VW_COST_AVOIDANCE_REPORT
UNION ALL
SELECT 
    'High-Risk Assets',
